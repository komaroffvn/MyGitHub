...
begin
  ...
  Ini.WriteString('Путь к БД','ConStr', Form6.Edit1.Text);
  Ini.Free;
   if Form6.Edit1.Text=''
   then
    MessageDlg('Файл БД не найден. Выберите файл БД',mtError,[mbOk],0)
    else
     begin
...
begin
  Ini:= TIniFile.Create(ExtractFilePath(ParamStr(0))+'setting.ini');
  Form6.Edit1.Text:= Ini.ReadString('Путь к БД','ConStr', Form6.Edit1.Text);
  Ini.Free;
end;
